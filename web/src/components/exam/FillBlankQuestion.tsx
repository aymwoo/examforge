  ...
  if (blankCount === 0) {
      return (
        <div className="w-full" data-question-id={id}>
            <p className="mb-4 text-ink-900 leading-relaxed whitespace-pre-wrap text-lg">{content}</p>
            <div className="mt-2">
                <textarea 
    ...
    <div className="text-lg leading-loose text-ink-900" data-question-id={id}>
      {parts.map((part, index) => {
        ...
        if (regex.test(part)) {
          ...
          return (
            <span key={index} className="inline-block mx-1 align-baseline relative group">
              <input
                type="text"
                disabled={disabled}
